...
{{ range .Methods }}
{{- if eq .Type 1 }}
func (s *{{ .Service }}) {{ .Name }}(ctx context.Context, req {{ if eq .Request $s1 }}*emptypb.Empty{{ else }}*pb.{{ .Request }}{{ end }}) ({{ if eq .Reply $s1 }}*emptypb.Empty{{ else }}*pb.{{ .Reply }}{{ end }}, error) {
    {{ if eq .Reply $s1 }}
	&emptypb.Empty{}
	{{ else }}
	reply := &pb.{{ .Reply }}{}
...
func (s *Service) create() ([]byte, error) {
...
func (s *Service) append(ms []*Method) ([]byte, error) {
	s.Methods = ms
	buf := new(bytes.Buffer)
	tmpl, err := template.New("append").Parse(appendTemplate)
	if err != nil {
		return nil, err
	}
	if err := tmpl.Execute(buf, s); err != nil {
