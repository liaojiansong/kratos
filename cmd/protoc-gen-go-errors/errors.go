...
func generateDefault(gen *protogen.Plugin, file *protogen.File) *protogen.GeneratedFile {
	if len(file.Enums) == 0 {
		return nil
	}
	filename := file.GeneratedFilenamePrefix + "_default_errors.pb.go"
	g := gen.NewGeneratedFile(filename, file.GoImportPath)
	g.P("// Code generated by protoc-gen-go-errors. DO NOT EDIT.")
	g.P()
	g.P("package ", file.GoPackageName)
	...
	generateDefaultContent(gen, file, g)
	return g
}

func generateDefaultContent(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile) {
	...
	for _, enum := range file.Enums {
		if !genErrorsDefault(gen, file, g, enum) {
			index++
		}
	}
...
func prueComment(s string) string {
	if len(s) == 0 {
		return ""
	}
...
func genErrorsDefault(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile, enum *protogen.Enum) bool {
	defaultCode := proto.GetExtension(enum.Desc.Options(), errors.E_DefaultCode)
	code := 0
	if ok := defaultCode.(int32); ok != 0 {
		code = int(ok)
	...
	for _, v := range enum.Values {
		...
		err := &errorInfo{
			...
			HTTPCode:    enumCode,
			HasComment:  len(comment) > 0,
			CommentPure: prueComment(comment),
		}
		ew.Errors = append(ew.Errors, err)
	}
	if len(ew.Errors) == 0 {
